...
export default abstract class Parameter<T extends ParameterValueType> {
    ...
    constructor(props: ParameterProps<T>) {
        ...
        if (this.setValueCallback) this.setValueCallback(value);
    }

    setValue(value: T) {
        this.value = value;

        if (this.inputHandle) this.inputHandle(value);
    }
    ...
    updateWithAutomation(time: number) {
        if (this.automationCallback) this.setValue(this.automationCallback(time));
    }
