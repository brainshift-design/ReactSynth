...
export default class FilterNode extends AudioNode<FilterNodeProps> {
    ...
    override componentWillUnmount() {
        ...
        this.context.setFilterTypes(this.context.filterTypes.filter((type) => type.id != this.id));
    }
    ...
    protected override initAudioNode() {
        const {
            data: { frequency, detune, Q, gain, type },
        } = this.props;
    ...
    override updateAudioParam(key: keyof FilterNodeProps['data'], value: number) {
        if (key === 'type') {
            const node = this.audioNode as globalThis.BiquadFilterNode;
            if (node) {
                node.type = (FilterNode.filterTypes[value]?.value as BiquadFilterType) ?? 'lowpass';
            }
        } else {
            ...
            <>
                ...
                <div className={styles.nodeContent}>
                    ...
                    <NumberKnob
                        ...
                        getCurvedTick={(val) =>
                            getValueCurve(val, 0, 1, valueCurve.Q, (v) => 1 - v)
                        }
                        ticks={knobTicks.Q}
