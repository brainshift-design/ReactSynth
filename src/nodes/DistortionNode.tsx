...
export default class DistortionNode extends AudioNode<DistortionNodeProps> {
    ...
    protected override initAudioNode() {
        ...
        if (audioNode)
            audioNode.oversample = DistortionNode.oversampleTypes[oversample]
                .value as OverSampleType;
    }

    override updateAudioParam(key: keyof DistortionNodeProps['data'], value: number) {
        super.updateAudioParam(
            key,
            key === 'oversample' ? (DistortionNode.oversampleTypes[value]?.value ?? 'none') : value
        );
        ...
        if (node) node.curve = createDistortionCurve(amount);
    }
