...
import { ClassContext, ClassContextProps } from '../contexts/ClassContext';
...
> extends Component<T, S> {
    ...
    get type() {
        return getTypeName(this.constructor as abstract new (props: unknown) => this);
    }
    get data() {
    ...
    update(data: Partial<T['data']>) {
        this.context.setNodes((nodes) =>
            nodes.map((node) =>
                node.id == this.props.id ? { ...node, data: { ...node.data, ...data } } : node
            )
        );
    }
